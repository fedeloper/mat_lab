...
def LPM_filter(X, Y):
    ...
    treeX = KDTree(X)
    _, neighborX = treeX.query(X, k=numNeigh1)
    treeY = KDTree(Y)
    _, neighborY = treeY.query(Y, k=numNeigh1)

    idx, C = LPM_cosF(neighborX, neighborY, lambda1, vec, d2, tau1, numNeigh1)

    if len(idx) >= numNeigh2 + 4:
        treeX2 = KDTree(X[idx,:])
        _, neighborX2 = treeX2.query(X, k=numNeigh2)
        treeY2 = KDTree(Y[idx,:])
        _, neighborY2 = treeY2.query(Y, k=numNeigh2)
