...
class LoFTREncoderLayer(nn.Module):
    ...
    def forward(self, x:Tensor, source:Tensor, x_mask: Optional[Tensor] =None, source_mask:Optional[Tensor] = None ):
        ...
        Args:
            x (Tensor): [N, L, C]
            source (Tensor): [N, S, C]
            x_mask (Tensor): [N, L] (opctional)
            source_mask (Tensor): [N, S] (optional)
        """
        bs = x.shape[0]
...
class TopicFormer(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.seed_tokens = nn.Parameter(torch.randn(config['n_topics'], config['d_model'],dtype=torch.float))
    ...
    def sample_topic(self, prob_topics:Tensor, topics:Tensor, L: int)-> Tuple[Tensor,Tensor,int]:
        """
        Args:
            topics (Tensor): [N, L+S, K]
        """
        ...
        return sampled_topics0, sampled_topics1,1

    def reduce_feat(self, feat:Tensor, topick:Tensor, N:int, C:int)-> Tuple[Tensor,Tensor,Tensor]:
        len_topic = topick.sum(dim=-1).int()
        max_len = len_topic.max().item()
        selected_ids = topick.to(torch.bool)
        resized_feat = torch.zeros((N, int(max_len), C), dtype=torch.float32, device=feat.device)
        new_mask = torch.zeros_like(resized_feat[..., 0]).to(torch.bool)
        for i in range(N):
            new_mask[i, :len_topic[i]] = True
        resized_feat[new_mask, :] = feat[selected_ids, :]
        return resized_feat, new_mask, selected_ids

    def forward(self, feat0:Tensor, feat1:Tensor, mask0:Optional[Tensor]=None, mask1:Optional[Tensor]=None) -> Tuple[Tensor,Tensor,Tensor,Tensor]:
        """
        Args:
            feat0 (Tensor): [N, L, C]
            feat1 (Tensor): [N, S, C]
            mask0 (Tensor): [N, L] (optional)
            mask1 (Tensor): [N, S] (optional)
        """
        ...
        for i , layer in enumerate(self.layers): #range (self.layer_count):#enumerate( zip(self.layers, self.layer_names)):
        ...
        if sampled_topics[2] ==0:
            ...
            for k in range(s_topics0.shape[-1]):
                ...
                if (topick0.sum() > 0) and (topick1.sum() > 0):
                    ...
                    for idt,layer_ in enumerate(self.topic_transformers):
                        if idt < self.n_iter_topic_transformer*2:
                            new_feat0 = layer_(new_feat0, new_feat0, new_mask0, new_mask0)

                            new_feat1 = layer_(new_feat1, new_feat1, new_mask1, new_mask1)
...
class LocalFeatureTransformer(nn.Module):
    ...
    def forward(self, feat0:Tensor, feat1:Tensor, mask0:Optional[Tensor]=None, mask1:Optional[Tensor]=None) -> Tuple[Tensor,Tensor]:
        """
        Args:
            feat0 (Tensor): [N, L, C]
            feat1 (Tensor): [N, S, C]
