...
from Tester.tester import get_list, rotate_llh_map, get_sample, get_metrics_differentiable
...
def get_metrics_batch_new(crs_indices,mks0,mks1 , llh, size_search, size_patch,off_x,off_y, angle_rotation=0):
    loss= None
...
dataloader_val = DataLoader(test_dataset, batch_size=8, shuffle=False)
device =torch.device("cuda" if torch.cuda.is_available() else "cpu")
net = ResNet_VAE().to(device)  # Your model
criterion = nn.MSELoss()  # Your loss function
optimizer = torch.optim.AdamW(net.parameters(), lr=.01)  # Your optimizer
...
for epoch in range(num_epochs):
    ...
    for i, data in enumerate(tqdm(dataloader_train)):
        with autograd.detect_anomaly():
            ...
            loss = get_metrics_batch_new(idxs, mks0, mks1, llh, size_search, size_patch,off_x,off_y,
                                         ANGLE_ROTATION)
    ...
    with torch.no_grad():
        for i, data in tqdm(enumerate(dataloader_val, 0)):
                patch, search, llh, idxs, off_x, off_y = data
                patch, search = preprocess_patch_search(patch.to(torch.float), search.to(torch.float))
                patch = patch.to(device).detach()
                search = search.to(device).detach()
